...
def read_shard(path):
    ...
    with path.open('rb') as fp:
        ...
        spans = []
        for t in tensors:
            t.update(path=str(path.resolve()), abs_offset=data_offset + t['offset'])
            end = t['abs_offset'] + t['nbytes']
            if end > size:
                fail(f'{path}: truncated tensor {t["name"]}')
            spans.append((t['abs_offset'], end))
        spans.sort()
...
def payload(tensor):
    with open(tensor['path'], 'rb') as fp:
        fp.seek(tensor['abs_offset'])
        return read_exact(fp, tensor['nbytes'], tensor['name'])
...
def write_import(output, entries, records, provenance):
    output = Path(output)
    ...
    try:
        with os.fdopen(fd, 'wb') as out:
            ...
            for i, entry in enumerate(entries, 1):
                ...
                if entry.get('replacement') is not None:
                    ...
                    with open(src['path'], 'rb') as source:
                        source.seek(src['abs_offset'])
                        remaining = plan.nbytes
                        while remaining:
                            block = read_exact(source, min(remaining, 8 << 20), plan.name)
                            out.write(block)
                            digest.update(block)
                            remaining -= len(block)
                out.write(bytes(align(plan.nbytes) - plan.nbytes))
                written.append(dict(name=plan.name, source_name=src['name'], source_file=src['path'],
        ...
        with report_path.open('x') as fp:
            ...
            fp.flush()
            os.fsync(fp.fileno())
        # The output must never replace a file created while this long copy ran.
...
def verify_import(output):
    ...
    _, tensors = read_shard(output)
    expected = {t['name']: t for t in report['tensors']}
    if (len(expected) != len(tensors) or len(expected) != len(report['tensors']) or
            Path(output).stat().st_size != report['output_bytes']):
        fail('output does not match import manifest')
    with open(output, 'rb') as fp:
        if hashlib.sha256(read_exact(fp, report['header_bytes'], 'header')).hexdigest() != report['header_sha256']:
        ...
        for t in tensors:
            ...
            fp.seek(t['abs_offset'])
            digest = hashlib.sha256()
            remaining = t['nbytes']
            while remaining:
                block = read_exact(fp, min(remaining, 8 << 20), t['name'])
                digest.update(block)
                remaining -= len(block)
...
def main():
    ...
    _, published_tokens = load_tokenizer_records(paths[0])
    if published_tokens != tokens:
    ...
    try:
        ...
        provenance = dict(mode='source-aligned', source_revision=args.source_revision,
                          ...
                          input_files=[dict(path=str(p.resolve()), sha256=checksums[p.name]) for p in paths],
        ...
        if not args.dry_run:
            for path in paths:
                verify_file(path, checksums[path.name])
            write_import(args.out, entries, records, provenance)
    finally:
        db.close()
